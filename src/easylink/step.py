...
class ParallelStep(CompositeStep, BasicStep):
    ...
    def update_implementation_graph(
        ...
        if not self.config_key in step_config:
            ...
            self.graph = self._create_parallel_graph(step_config)
            self.slot_mappings = self._get_parallel_slot_mappings(sub_config)
    ...
    def _get_parallel_slot_mappings(
        self, parallel_steps: List[LayeredConfigTree]
    ) -> nx.MultiDiGraph:
        """Get the appropriate slot mappings based on the number of parallel steps
        and the non-self-edge input and output slots."""
        input_mappings = []
        self_edge_input_slots = {edge.input_slot for edge in self.self_edges}
        external_input_slots = self.input_slots.keys() - self_edge_input_slots
        for input_slot in self_edge_input_slots:
            input_mappings.append(
                SlotMapping(
                    "input", self.name, input_slot, f"{self.name}_parallel_1", input_slot
                )
            )
        for input_slot in external_input_slots:
            input_mappings.extend(
                [
                    SlotMapping(
                        ...
                        self.name,
                        input_slot,
                        f"{self.name}_parallel_{n+1}",
                        input_slot,
                    )
                    for n in range(len(parallel_steps))
                ]
            )
        output_mappings = [
            SlotMapping(
                "output", self.name, slot, f"{self.name}_parallel_{len(parallel_steps)}", slot
            )
            for slot in self.output_slots
        ]
        return {"input": input_mappings, "output": output_mappings}
