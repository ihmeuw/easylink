...
conf = SparkConf()
conf.setMaster(os.getenv("DUMMY_CONTAINER_SPARK_MASTER_URL"))
...
with open("/code/splink_model_params.json", "r") as f:
...
def prep_table_for_splink(df, dataset_name):
    df = df[common_cols].assign(dataset_name=dataset_name)
