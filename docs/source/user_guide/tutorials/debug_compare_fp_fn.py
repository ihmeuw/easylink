...
def clusters_to_links(clusters_df):
    ...
    links_df = filtered[
        ["Input Record Dataset_left", "Input Record ID_left", "Input Record Dataset_right", "Input Record ID_right"]
    ].copy()
    links_df.columns = ["Left Record Dataset", "Left Record ID", "Right Record Dataset", "Right Record ID"]
    links_df["Probability"] = 1.0
    return links_df

def unique_id_column(df, side="Left"):
    return df[f"{side} Record Dataset"].astype(str) + "_" + df[f"{side} Record ID"].astype(str)
...
def compute_fp_fn(results_dir: Path):
    ...
    input_data_files = {k: Path(v).resolve() for k, v in input_data_yaml.items() if Path(v).stem != 'known_clusters'}
    records = pd.concat([
    ...
    records["unique_id"] = records["Input Record Dataset"].astype(str) + "_" + records["Input Record ID"].astype(str)
    ...
    pred_links = pred_links.merge(
        ...
        records.add_suffix("_r"), on="unique_id_r", how="left"
    )

    # Compute False Positives
    false_positives = pred_links[
        pred_links["simulant_id_l"] != pred_links["simulant_id_r"]
    ]
    ...
    links["matched"] = links.apply(lambda row: (row["unique_id_l"], row["unique_id_r"]) in predictions_set, axis=1)
...
def show_deltas(old_df, new_df, label):
    ...
    def get_details(pairs, df, kind):
        return df.set_index(["unique_id_l", "unique_id_r"]).loc[list(pairs)].reset_index()[[
            "unique_id_l", "unique_id_r",
            *[f"{col}_l" for col in KEY_COLS],
            *[f"{col}_r" for col in KEY_COLS]
        ]].assign(type=kind).sort_values(["unique_id_l", "unique_id_r"])
